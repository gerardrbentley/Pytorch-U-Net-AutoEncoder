...
class GameFoldersDataset(torch.utils.data.Dataset):
    def __init__(self, root='/faim/datasets/per_game_screenshots/', train_or_val="train", transform=TT.ToTensor()):
        ...
        for game_folder in self.image_folders:
            sets.append(GameImagesDataset(root=os.path.join(
                root, game_folder), train_or_val=train_or_val, transform=transform))

        self.full_dataset = torch.utils.data.ConcatDataset(sets)
        if train_or_val == "val":
            to_chunk = int(0.2 * len(self.full_dataset))
            self.full_dataset, _ = torch.utils.data.random_split(
                self.full_dataset, [to_chunk, len(self.full_dataset) - to_chunk])
        self.transform = transform
...
if __name__ == "__main__":
    ...
    trainset = GameImagesDataset(
        root='/faim/datasets/mario_images', train_or_val='train', transform=None)
